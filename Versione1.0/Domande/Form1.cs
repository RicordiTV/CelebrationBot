...
namespace Domande
{
    public partial class Form1 : Form
    {
        ...
        {
            DialogResult result = MessageBox.Show("Vuoi attivare la versione Offline?", "Alert", MessageBoxButtons.YesNo);


            if (result == DialogResult.Yes)
            {
                timer5.Start();
                string text1 = @"offline.html";
                StreamWriter streamWriter1 = new StreamWriter(text1, false);
                streamWriter1.WriteLine("<h1>Benvenuto nella modalità Offline: </h1><br><br><h2>1) Apri il tuo OBS<br>2) Inserisci l'immagine di sfondo che desideri esempio image.png nel tuo software di steam<br><img src='image.png'><br>3) aggiungi una fonte di tipo testo<br>4) Flagga su leggi da file e seleziona il file risultato.txt<br><br>ps. Si informa che lo stile del font, il carattere e la posizione della scritta dovranno essere scelte in seguito dal utente manualmente</h2>");
                streamWriter1.Close();
                string currentDirectory = Environment.CurrentDirectory;
                // Costruisce il percorso completo del file desiderato
                string filePath3 = Path.Combine(currentDirectory, @"offline.html");
                Process.Start(filePath3.ToString());
        ...
        private void timer5_Tick_1(object sender, EventArgs e)
        {
            ...
            string contenuto = label1.Text;

            // Cria o arquivo .txt
            using (StreamWriter arquivo = new StreamWriter(nomefile))
            {
                arquivo.WriteLine(contenuto);
            }
